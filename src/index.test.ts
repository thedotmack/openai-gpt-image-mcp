...
jest.mock("openai", () => {
  ...
  class ImagesAPI {
    private mode: "generate" | "edit";
    private payloads: any[];
    constructor(mode: "generate" | "edit", payloads: any[]) {
      this.mode = mode;
  ...
  const __setGeneratePayloads = (payloads: any[]) => {
    (OpenAI as any).prototype.images = new ImagesAPI("generate", payloads);
    (AzureOpenAI as any).prototype.images = new ImagesAPI("generate", payloads);
  };
  const __setEditPayloads = (payloads: any[]) => {
    (OpenAI as any).prototype.images = new ImagesAPI("edit", payloads);
    (AzureOpenAI as any).prototype.images = new ImagesAPI("edit", payloads);
  };
...
jest.mock("fs", () => {
  ...
  const memFS: Record<string, any> = {};
  const mkdirSync = jest.fn((dirPath: string, opts?: any) => {
    // emulate recursive creation
    const parts = dirPath.split(path.sep);
    let cur = "";
    for (const p of parts) {
      if (!p) continue;
      cur = cur ? path.join(cur, p) : (path.isAbsolute(dirPath) ? path.sep + p : p);
      memFS[cur] = memFS[cur] || { type: "dir" };
    }
    return undefined;
  });
  const readFileSync = jest.fn((filePath: string, enc: string) => {
  ...
  const writeFileSync = jest.fn((filePath: string, data: any) => {
    memFS[filePath] = { type: "file", content: data.toString() };
  });
  const createReadStream = jest.fn((filePath: string) => {
    if (!(filePath in memFS) || memFS[filePath].type !== "file") {
  ...
  const promises = {
    writeFile: jest.fn(async (filePath: string, data: any) => {
...
jest.mock("fs/promises", () => {
  const mem: Record<string, any> = (jest.requireMock("fs") as any).__memFS;
  return {
    writeFile: jest.fn(async (filePath: string, data: any) => {
...
jest.mock("os", () => {
  ...
  return {
    homedir: () => home,
    tmpdir: () => tmp,
    __setHome: (h: string) => (home = h),
    __setTmp: (t: string) => (tmp = t),
  };
});
...
describe("Helper: loadEnvFile", () => {
  test("loads key=value pairs, trims whitespace, ignores comments, supports quotes", () => {
    const fs: any = jest.requireMock("fs");
  ...
  test("warns but does not throw when file cannot be read", () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining("Warning: Could not read environment file"),
      expect.any(Error)
    );
  });
});
...
describe("Helper: getDefaultImageDirectory", () => {
  test("returns Desktop/Generated_Images for linux/darwin and creates it", () => {
    const os: any = jest.requireMock("os");
    const fs: any = jest.requireMock("fs");
    os.__setHome("/home/alice");
    const fn = mod.getDefaultImageDirectory ?? mod["getDefaultImageDirectory"];
    const dir = fn();
    expect(dir).toBe(path.join("/home/alice", "Desktop", "Generated_Images"));
  ...
  test("falls back to os.tmpdir() when mkdir fails", () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining("Warning: Could not create default image directory"),
      expect.any(Error)
    );
  });
});
...
describe("Schema: createImageSchema", () => {
  ...
  test("validates absolute file_output when output is 'file_output'", () => {
    const schema = mod.createImageSchema ?? mod["createImageSchema"];
    expect(() =>
      schema.parse({ prompt: "x", output: "file_output", file_output: "relative/path.png" })
    ).toThrow(/absolute path/i);
    // Valid absolute (unix)
    expect(() =>
      schema.parse({ prompt: "x", output: "file_output", file_output: "/abs/path.png" })
    ).not.toThrow();
    // Valid absolute (win)
    expect(() =>
      schema.parse({ prompt: "x", output: "file_output", file_output: "C:\\\\abs\\\\path.png" })
    ).not.toThrow();
  });
  ...
  test("enforces model-specific rules: dall-e-3 only n=1 and restricts params", () => {
    const schema = mod.createImageSchema ?? mod["createImageSchema"];
    expect(() =>
      schema.parse({ prompt: "x", model: "dall-e-3", n: 2 })
    ).toThrow(/Invalid parameter combination/i);
    expect(() =>
      schema.parse({ prompt: "x", model: "dall-e-3", background: "transparent" as any })
    ).toThrow(/Invalid parameter combination/i);
    expect(() =>
      schema.parse({ prompt: "x", model: "dall-e-3", size: "1792x1024" })
    ).not.toThrow();
  });
  ...
  test("enforces model-specific rules: dall-e-2 restrictions and size/quality", () => {
    const schema = mod.createImageSchema ?? mod["createImageSchema"];
    expect(() =>
      schema.parse({ prompt: "x", model: "dall-e-2", quality: "hd" as any })
    ).toThrow(/Invalid parameter combination/i);
    expect(() =>
      schema.parse({ prompt: "x", model: "dall-e-2", size: "1024x1024" })
    ).not.toThrow();
    expect(() =>
      schema.parse({ prompt: "x", model: "dall-e-2", style: "vivid" as any })
    ).toThrow(/Invalid parameter combination/i);
  });
  ...
  test("gpt-image-1: forbids response_format/style and validates quality/size", () => {
    ...
    expect(() => schema.parse({ prompt: "x", style: "vivid" as any })).toThrow(/Invalid parameter combination/i);
...
describe("Schema: editImageSchema", () => {
  test("accepts single or multiple image inputs for gpt-image-1", () => {
    ...
    expect(() =>
      schema.parse({ image: "/abs/a.png", prompt: "p" })
    ).not.toThrow();
    expect(() =>
      schema.parse({ image: ["/abs/a.png", "/abs/b.png"], prompt: "p", model: "gpt-image-1" })
    ).not.toThrow();
  });
  ...
  test("dall-e-2: rejects multiple images and long prompts; restricts quality/size", () => {
    const schema = mod.editImageSchema ?? mod["editImageSchema"];
    expect(() =>
      schema.parse({ image: ["/abs/a.png", "/abs/b.png"], prompt: "p", model: "dall-e-2" })
    ).toThrow(/Invalid parameter combination/i);
    expect(() =>
      schema.parse({ image: "/abs/a.png", prompt: "x".repeat(1001), model: "dall-e-2" })
    ).toThrow(/Invalid parameter combination/i);
    expect(() =>
      schema.parse({ image: "/abs/a.png", prompt: "ok", model: "dall-e-2", quality: "standard" })
    ).not.toThrow();
    expect(() =>
      schema.parse({ image: "/abs/a.png", prompt: "ok", model: "dall-e-2", size: "256x256" })
    ).not.toThrow();
  });
  ...
  test("file_output absolute path enforcement when output is file_output", () => {
    const schema = mod.editImageSchema ?? mod["editImageSchema"];
    expect(() =>
      schema.parse({ image: "/abs/a.png", prompt: "p", output: "file_output", file_output: "rel/out.png" })
    ).toThrow(/absolute path/i);
    expect(() =>
      schema.parse({ image: "/abs/a.png", prompt: "p", output: "file_output", file_output: "/abs/out.png" })
    ).not.toThrow();
  });
});
...
describe("Handler: create-image behavior", () => {
  test("gpt-image-1 base64 response under 1MB returns base64 images", async () => {
    const { __setGeneratePayloads } = await import("openai") as any;
  ...
  test("dall-e-3 with response_format=url returns URLs", async () => {
    ...
    const lines = res.content.map((c: any) => c.text);
...
describe("Handler: edit-image behavior", () => {
  ...
  test("writes multiple outputs with indexed filenames when n>1 and file_output chosen", async () => {
    const { __setEditPayloads } = await import("openai") as any;
    ...
    const out = res.content.map((c: any) => c.text);
